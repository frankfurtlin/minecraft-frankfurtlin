...
public class BackPackScreenHandler extends ScreenHandler {
    ...
    private BackPackScreenHandler(ScreenHandlerType<?> type, int syncId, PlayerInventory playerInventory, int rows) {
        this(type, syncId, playerInventory, new SimpleInventory(9 * rows), rows);
    }


    public static BackPackScreenHandler createGeneric9x6(int syncId, PlayerInventory playerInventory) {
        return new BackPackScreenHandler(ScreenHandlerType.GENERIC_9X6, syncId, playerInventory, 6);
    }

    public static BackPackScreenHandler createGeneric9x6(int syncId, PlayerInventory playerInventory, Inventory inventory) {
        return new BackPackScreenHandler(ScreenHandlerType.GENERIC_9X6, syncId, playerInventory, inventory, 6);
    }

    public BackPackScreenHandler(ScreenHandlerType<?> type, int syncId,
    ...
    public void onSlotClick(int slotId, int clickData, SlotActionType actionType, PlayerEntity playerEntity)
    {
        ...
        if (slotId >= 0)  // slotId < 0 are used for networking internals
        {
            ...
            if(stack.getItem() instanceof BackPack ||
                    stack.getItem() instanceof EnderPack ||
    ...
    public ItemStack quickMove(PlayerEntity player, int index) {
        ...
        if(testItem instanceof BackPack || SHULKER_BOXES.contains(testItem))
        {
